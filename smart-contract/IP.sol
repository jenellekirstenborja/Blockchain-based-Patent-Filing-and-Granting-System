...
contract IntellectualPropertySystem {
    ...
    modifier onlyAdmin() {
        require(roles[msg.sender] == Role.Admin, "Only admin can access this function"); // access control
        _;
    }
    ...
    function applyForPatent(
        ...
        require(!hasBeenApplied[keccak256(abi.encodePacked(msg.sender, _title))], "This application has already been filed");
    ...
    function grantPatent(uint64 _applicationNumber) external onlyAdmin {
        require(isPatentAppliedFor[_applicationNumber], "Application does not exist");
        require(!isPatentGranted[_applicationNumber], "Patent has already been granted");
        appliedPatents[appliedIndex[_applicationNumber]].patentNumber = ctrPatentNumber; // unique patent number
        appliedPatents[appliedIndex[_applicationNumber]].publicationDate = block.timestamp; // current timestamp
    ...
    // View Application
    function viewApplication(uint64 _applicationNumber) external view returns (Patent memory) {
        require(isPatentAppliedFor[_applicationNumber], "Application does not exist");
        return appliedPatents[appliedIndex[_applicationNumber]];
    }

    // View Patent
    function viewPatent(uint64 _applicationNumber) external view returns (Patent memory) {
        require(isPatentGranted[_applicationNumber], "Application has not been granted patent status");
